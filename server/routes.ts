...
import { extractAddressFromText, extractAddressesFromText, validateDeclaredAddress, findBestMatchingAddress } from "./services/address.service.js";
...
const RESIDENCE_DOC_TYPE = "residence";
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post(api.documents.upload.path, requireAuth, async (req, res) => {
    try {
      ...
      } else if (input.type === RESIDENCE_DOC_TYPE) {
        try {
          const enrollment = await storage.getEnrollment(id);
          const declaredAddress = enrollment?.address?.trim();
          const { text, confidence } = await extractTextFromDataUrl(input.base64Content);

          if (confidence < 35) {
            ocrData = {
              status: "REVISAO_MANUAL",
              enderecoExtraido: "campo_nao_identificado",
              confianca: Number(confidence.toFixed(2)),
            };
            await storage.updateEnrollmentStatus(
              id,
              "files_pending",
              "Revisão manual necessária: qualidade do comprovante baixa para leitura automática do endereço. Reenvie uma imagem mais nítida ou aguarde análise manual."
            );
            analysePendente = {
              tipo: 'nao_identificado',
              campos_nao_identificados: ["endereco"],
              observacoes: [
                `OCR com baixa confiança (${Math.round(confidence)}%).`,
                "Não foi possível validar o endereço com segurança.",
                "Reenvie o comprovante com imagem inteira, sem cortes e com melhor nitidez.",
              ],
            };
          } else {
          
            // Extrai múltiplos endereços candidatos
            const enderecos = extractAddressesFromText(text);

            if (!declaredAddress) {
              const enderecoExtraido = enderecos.length > 0 ? enderecos[0] : "campo_nao_identificado";
              ocrData = {
                status: "REVISAO_MANUAL",
                enderecoExtraido,
              };
              await storage.updateEnrollmentStatus(
                id,
                "files_pending",
                "Endereço não informado no cadastro. Preencha o endereço e reenvie o comprovante de residência."
              );
              analysePendente = {
                tipo: 'nao_identificado',
                campos_nao_identificados: ["endereco"],
                observacoes: [
                  "Endereço não informado no cadastro. Preencha o endereço antes de reenviar o comprovante.",
                ],
              };
              ...
              await storage.updateEnrollmentStatus(
                id,
                "files_pending",
                "Revisão manual necessária: não foi possível identificar o endereço no comprovante de residência."
              );
              analysePendente = {
                tipo: 'nao_identificado',
                campos_nao_identificados: ["endereco"],
                observacoes: [
                  "Não foi possível identificar o endereço no comprovante de residência.",
                  "Reenvie o comprovante com melhor qualidade (imagem inteira e legível).",
                ],
              };
            } else {
              // Encontra o melhor endereço que corresponde ao declarado
              const bestMatch = findBestMatchingAddress(declaredAddress, enderecos);
              
              if (!bestMatch || !bestMatch.corresponde) {
                // Nenhum endereço correspondeu bem
                const firstAddress = bestMatch?.enderecoExtraido || enderecos[0];
                const score = bestMatch?.scoreCorrespondencia ?? 0;
                ocrData = {
                  status: "REPROVADO",
                  enderecoExtraido: firstAddress,
                };
                await storage.updateEnrollmentStatus(
                  id,
                  "files_pending",
                  "Comprovante de residência divergente do endereço informado no cadastro. Verifique e reenvie o documento correto."
                );
                analysePendente = {
                  tipo: 'nao_correspondente',
                  campos_nao_identificados: ["endereco"],
                  observacoes: [
                    "O endereço extraído do comprovante não confere com o endereço informado no cadastro.",
                    `Score de correspondência: ${Math.round(score * 100)}%`,
                    `Endereços testados: ${enderecos.length} candidato(s)`,
                  ],
                };
              } else {
                // Endereço correspondeu!
                ocrData = {
                  status: "APROVADO",
                  enderecoExtraido: bestMatch.enderecoExtraido,
                };
              }
            }
          }
        } catch (ocrErr: any) {
          console.error(`[OCR Residence] Falha ao processar documento ${input.name}:`, ocrErr?.message ?? ocrErr);
          ocrData = {
            status: "REVISAO_MANUAL",
          };
          await storage.updateEnrollmentStatus(
            id,
            "files_pending",
            "Revisão manual necessária: falha no processamento OCR do comprovante de residência."
          );
          analysePendente = {
            tipo: 'nao_identificado',
            campos_nao_identificados: ["endereco"],
            observacoes: [
              "Falha ao validar automaticamente o endereço no comprovante de residência.",
              ocrErr?.message ?? "Erro desconhecido",
            ],
          };
        }
      }
  ...
  app.post(api.enrollments.submit.path, requireAuth, async (req, res) => {
    ...
    if (!enrollment.address) {
      return res.status(422).json({ message: "Informe o endereço antes de enviar a inscrição." });
    }
